...
class Tracker:
    """Tracker class. Given a trained density estimator, searches for object.
...
class SamplingTracker(Tracker):
    ...
    def _generate_points_to_check(self, num_samples: int):
        self.points_to_check = self.density_estimator.sample(num_samples)    
    
    def simulate_tracking(self, true_point: Tensor):
        if self.points_to_check is None:
           self._generate_points_to_check(1000)
        return super().simulate_tracking(true_point)
